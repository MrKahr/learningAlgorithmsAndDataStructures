...
## 1.1-4
The shortest path problem and the travelling salesperson problems requires computing an optimal path between a source and a destination, but they differ in problem complexity.  The shortest path problem requires iteratively computing the lowest cost path between two nodes. Djisktra and Bellman proved that the computation could be completed in polynomial time.  TSP requires finding an optimal permutation of nodes s.t. each node is visited exactly once by an algorithm. Thus, it has to consider all possible permutations of nodes to find and compute an optimal path. The problem is therefore NP-hard. 
